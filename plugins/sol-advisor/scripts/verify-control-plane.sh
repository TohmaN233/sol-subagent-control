...
[ "$(jq -r '.version' "$manifest")" = 0.7.5 ] || fail "native manifest version drifted"
...
jq -e '.version == 5 and .global.enabled == true and .global.allow_direct_api == false and (.scenarios | not)' "$config" >/dev/null || fail "default global switches or schema are unsafe"

...
jq empty "$manifest"
[ "$(jq -r '.version' "$manifest")" = 0.7.4 ] || fail "manifest version is not 0.7.4"
...
pass "manifest JSON, v0.7.4 release, and selective-routing language"
...
printf '%s\n' "VERIFY PASSED: Sol Advisor v0.7.4 selective routing checks completed in $tmp_dir"

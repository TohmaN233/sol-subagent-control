...
test('Strict profile rejects changed binary and strips inherited execution/config overrides', async t => {
...
test('Strict catalog enforcement is by path, explicit injection is separate, and policy changes invalidate launch', async t => {
  const f = await fixture(t); const source = join(f.root, 'source', 'SKILL.md'); const cwd = join(f.root, 'workspace'); await mkdir(cwd);
...
test('unqualified administrative scope, shadowed source and unpinned ambient grant fail closed', async t => {
  const f = await fixture(t); const source = join(f.root, 'SKILL.md');
  ...
  assert.deepEqual(policy.tools(), []); await cleanupCodexProfile(f.profile);
});

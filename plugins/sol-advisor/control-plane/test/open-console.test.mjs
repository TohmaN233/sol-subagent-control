...
test('one-click console CLI defaults to browser open and validates options', () => {
  assert.deepEqual(parseOptions([]), { port: 0, open: true, help: false });
  assert.deepEqual(parseOptions(['--port', '54321', '--no-open']), {
    port: 54321, open: false, help: false,
  });
  assert.deepEqual(parseOptions(['--help']), { port: 0, open: true, help: true });
  assert.throws(() => parseOptions(['--port', '70000']), /between 0 and 65535/);

...
export function analyzeSkillDependencies(snapshot) {
  ...
  for (const [path, bytes] of Object.entries(snapshot.files)) {
    ...
    if (/[A-Za-z]:[\\/]|\/(?:Users|home|usr|opt|etc)\//.test(text) || text.includes(snapshot.root)) unresolved.push({ code: 'SOURCE_LINKED_PATH', path, origin: 'observed' });
  ...
  return { requirements, references, unresolved, classification: unresolved.some(item => item.code === 'SOURCE_LINKED_PATH') ? 'source_linked' : unresolved.length || requirements.executables.length || requirements.environment.length ? 'external_requirements' : 'self_contained_candidate' };
}
